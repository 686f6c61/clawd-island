    ...
    init(settings: AppSettings, store: IslandStore) {
        ...
        selection.$pane
            ...
            .sink { [weak self] pane in
                self?.window?.title = "\(pane.title) — Claude Island"
                self?.settingsToolbar.selectedItemIdentifier = .init(pane.rawValue)
            }
    ...
    private var header: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                HStack(spacing: 6) {
                    Circle()
                        .fill(IslandPalette.statusColor(
                            store.activeSession?.status ?? .idle,
                            idle: IslandPalette.menuIdle
                        ))
                        .frame(width: 7, height: 7)
                    Text(store.activeSession?.status.label ?? "Ready")
                        .foregroundStyle(IslandPalette.statusColor(
                            store.activeSession?.status ?? .idle,
                            idle: IslandPalette.menuIdle
                        ))
...
private struct MenuSeparator: View {
    var body: some View {
        Rectangle()
            .fill(IslandPalette.menuSeparator)
            .frame(height: 1)
    }
}
